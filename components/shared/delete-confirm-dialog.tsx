  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl border border-brand-border bg-brand-bg-secondary p-5 shadow-2xl">
        <h3 className="text-base font-semibold text-brand-text-primary">{t("deleteConfirmTitle")} &ldquo;{name}&rdquo;?</h3>
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button type="button" onClick={onConfirm} className="flex-1 rounded-xl bg-destructive py-2.5 text-sm font-semibold text-destructive-foreground hover:opacity-90 transition-opacity">{t("delete")}</button>
        </div>
      </div>
    </div>
  )
